# MTS Specimens Data Tansformer
# Author: madihsiang(R07)

from openpyxl import load_workbook
from PyQt5.QtWidgets import (
    QGraphicsView, QGraphicsScene, QApplication, QGraphicsPixmapItem, QProgressBar)
...
def header(data):
    ...
    model.setHorizontalHeaderLabels(
        list(map(str, data.keys())))  # map可以直接將list裡面的型態轉型
...
def compRePlot():
    try:
        ...
        for i in range(len(exRatio)):
            ...
            newComp[exRatio[i]] = comp[exRatio[i]][(
                comp[exRatio[i]] < avg*(1+delRatio)) & (comp[exRatio[i]] > avg*(1-delRatio))]

        # 新的pd加上新的平均值
        newCompAvg = pd.concat([newComp, pd.DataFrame(
            newComp.mean()).rename(columns={0: "平均"}).T])
        ...
        if True not in np.isnan(list(newCompAvg.iloc[-1])):
            ...
            createLabels(A)
            plt.savefig(path+'\\Compressive Strength-剔除歧異值.png',
                        bbox_inches='tight')

            figName = path+"\\Compressive Strength-剔除歧異值.png"
            layout_Figure(lab4, figName, scene4,
                          w.graphicsView_4, scale=[1, 1])
...
def EMRePlot():
    try:
        ...
        for i in range(len(exRatio)):
            ...
            avg = EM_avg.iloc[-1][i]
            newEM[exRatio[i]] = EM[exRatio[i]][(
                EM[exRatio[i]] < avg*(1+delRatio)) & (EM[exRatio[i]] > avg*(1-delRatio))]

        # 新的pd加上新的平均值
        newEMAvg = pd.concat([newEM, pd.DataFrame(
            newEM.mean()).rename(columns={0: "平均"}).T])
        ...
        if True not in np.isnan(list(newEMAvg.iloc[-1])):
            ...
            createLabels(B)
            plt.savefig(path+'\\Elastic modulus-剔除歧異值.png',
                        bbox_inches='tight')

            figName = path+"\\Elastic modulus-剔除歧異值.png"
            layout_Figure(lab5, figName, scene5,
                          w.graphicsView_5, scale=[1, 1])
...
def plot():
    ...
    if w.PathTxt.toPlainText() == "":
        QMessageBox.critical(w, 'Error', "沒有選檔案!",
    ...
    if w.Cylinder.isChecked():
        ...
        QMessageBox.critical(w, 'Error', "沒有選試體尺寸!",
                             QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
        return

    # 判斷是否有輸入配比數量
    if w.inputWidget.rowCount() == 0:
        QMessageBox.critical(w, 'Error', "沒有輸入配比數量!",
                             QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
        return

    # 判斷是否有選擇存檔路徑
    if w.savePathTxt.toPlainText() == "":
        QMessageBox.critical(w, 'Error', "沒有選存檔路徑!",
                             QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
        return

    # 判斷是否有輸入存檔檔名
    if w.saveFileName.text() == "":
        QMessageBox.critical(w, 'Error', "沒有輸入存檔名稱!",
                             QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
        return
    ...
    for i in range(w.inputWidget.rowCount()):
        ...
        if w.inputWidget.item(i, 0) == None:
            QMessageBox.critical(
                w, 'Error', "配比種類沒有完整輸入!", QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
            return
        ...
        try:
            if float(w.inputWidget.item(i, 1).text()) == 0:
                QMessageBox.critical(
                    w, 'Error', "配比數量不能為零!", QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
                return
            if float(w.inputWidget.item(i, 1).text()) != int(float(w.inputWidget.item(i, 1).text())):
                QMessageBox.critical(
                    w, 'Error', "配比數量不能為小數!", QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
                return
            if float(w.inputWidget.item(i, 1).text()) < 0:
                QMessageBox.critical(
                    w, 'Error', "配比數量必須為正整數!", QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
                return
        except:
            QMessageBox.critical(w, 'Error', "配比數量必須為數字，不能為文字!",
                                 QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
            return
    ...
    if len(set(exRatio)) != len(exRatio):
        QMessageBox.critical(w, 'Error', "配比種類名稱重複!",
                             QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
        return
    ...
    n = len(fileRead)
    if sum(exNum) != n:
        QMessageBox.critical(w, 'Error', "試體個數不符合，請重新確認!",
                             QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
        return
    ...
    for i in range(len(fileRead)):
        ...
        for j in range(len(eachLineDataList)):
            ...
            # 除以夾具距離(100mm)  # LVDT1mm = 實際5.384mm
            strain = abs((D1+D2)/2)/100*5.384
    #         strain = (D1+D2)/200*10
    ...
    def Process_NoEM(n, stressDict, strainDict, PathFile, Savefile, exRatio):
        ...
        # DataFrame
        comp = pd.DataFrame(dict([(k, pd.Series(v))
                            for k, v in maxStressDict.items()]))
        comp = comp.rename(
            index={0: "第一顆", 1: "第二顆", 2: "第三顆", 3: "第四顆", 4: "第五顆(以下以此類推)"})
        comp = pd.concat(
            [comp, pd.DataFrame(comp.mean()).rename(columns={0: "平均"}).T])
        ...
        worksheet1.insert_image(
            1, 10, path+'\\Compressive Strength.png', {'x_scale': 1, 'y_scale': 1})
        workbook.close()  # 保存並關閉
    ...
    def Process_withEM(n, stressDict, strainDict, PathFile, Savefile, exRatio):
        ...
        for i in range(n):
            ...
            xmax = np.array(
                Strain_i_List[LowerPointIndex:UpperPointIndex]).reshape(-1, 1)
            ymax = np.array(
                Stress_i_List[LowerPointIndex:UpperPointIndex]).reshape(-1, 1)
            ...
            # 回歸錯誤的除錯
            try:
                popt, pcov = curve_fit(linear_regression, xmaxcf, ymaxcf)
            except:
            ...
            # 繪大圖 -> 預覽用
            axesPlot(ax[i, 0], ax[i, 1], x, y, i,
                     Strain_i_List, ex, xmax, ymax, popt)
        ...
        if len(errorList) != 0:
            for each in errorList:
                QMessageBox.critical(
                    w, 'Error', "配比 "+each[0]+" 的第"+each[1]+"個數據無法回歸!")
        ...
        EM = EM.rename(index={0: "第一顆", 1: "第二顆",
                       2: "第三顆", 3: "第四顆", 4: "第五顆(以下以此類推)"})
        EM_avg = pd.concat([EM[EM > 0], pd.DataFrame(
            EM[EM > 0].mean()).rename(columns={0: "平均"}).T])
        ...
        for i in range(len(fileRead)):
            ...
            worksheet1.write(20*i+3, 1, 12000 *
                             (max(stressDict['data'+str(i+1)]))**0.5)
            ...
            worksheet1.insert_image(
                20*i, 6, path+'\\fig_'+str(i+1)+'.png', {'x_scale': 0.65, 'y_scale': 0.65})
        ...
        worksheet3.insert_image(
            1, 10, path+'\\Elastic modulus.png', {'x_scale': 1, 'y_scale': 1})
    ...
    if w.Cube.isChecked():
        ...
        lab1.setStyleSheet(
            "QLabel{color:rgb(0,0,0,255);background-color:rgb(255,255,255,255);font-size:50px;font-weight:normal;font-family:Arial;}")
